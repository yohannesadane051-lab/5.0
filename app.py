...
def get_user_tests(username):
    try:
        ...
        for r in rows:
            if r.get("username") == username:
                ...
                user_tests.append({
                    ...
                    "created": created_date if created_date else created_str,
                    "mode": r.get("mode", ""),
        ...
        def get_sortable_date(test_obj):
            created = test_obj.get("created")
            if isinstance(created, datetime):
                return created
            elif isinstance(created, str):
                try:
                    # Try to parse if it's a string
                    for fmt in ["%Y-%m-%d %H:%M:%S.%f", "%Y-%m-%d %H:%M:%S", "%m/%d/%Y %H:%M:%S", "%d/%m/%Y %H:%M:%S"]:
                        try:
...
if "hide_create_test_elements" not in st.session_state:
    st.session_state.hide_create_test_elements = False
...
def navigate_to(page):
    ...
    if page == "test":
        ...
        st.session_state.hide_create_test_elements = True
    st.rerun()
        ...
        if not user_tests:
            ...
            if incomplete_tests:
                ...
                for i, test in enumerate(incomplete_tests):
                    with st.expander(f"⚠️ Incomplete Test {i+1}: {test.get('mode', 'Unknown')} Mode", expanded=(i==0)):
                        formatted_date = format_date_for_display(test.get("created"))
            ...
            if completed_tests:
                ...
                for i, test in enumerate(completed_tests):
                    ...
                    formatted_date = format_date_for_display(test.get("created"))
                    with st.expander(f"Test {i+1}: {test.get('mode', 'Unknown')} Mode - Score: {test['score']}/{test['total_questions']} - {formatted_date}"):
                        st.write(f"**Date:** {formatted_date}")
                        st.write(f"**Mode:** {test.get('mode', 'Unknown')}")
...
if st.session_state.page == "create":
    ...
    # ONLY SHOW THESE ELEMENTS ON THE CREATE TEST PAGE
    # They will be hidden during actual test sessions
    if not st.session_state.get("hide_create_test_elements", False):
        if mode == "Test":
    ...
    with col2:
        if st.button("Start Test", use_container_width=True):
            if len(pool) < num_q:
                ...
                # Reset the flag when starting a new test
                st.session_state.hide_create_test_elements = False
...
if st.session_state.page == "test":
    # Reset the flag when entering test mode
    st.session_state.hide_create_test_elements = True
...
if st.session_state.page == "test_review":
    # Reset the flag for review mode
    st.session_state.hide_create_test_elements = True
...
if st.session_state.page == "review":
    # Reset the flag for review page
    st.session_state.hide_create_test_elements = True
