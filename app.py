        ...
        def get_sortable_date(test_obj):
            ...
            try:
                ...
                try:
                    ...
                    for fmt in ["%Y-%m-%d %H:%M:%S", "%Y-%m-%d", "%m/%d/%Y %H:%M:%S", "%Y-%m-%dT%H:%M:%S"]:
                        try:
            ...
            if incomplete_tests:
                ...
                for i, test in enumerate(incomplete_tests):
                    with st.expander(f"⚠️ Incomplete Test {i+1}: {test.get('mode', 'Unknown')} Mode", expanded=(i==0)):
                        # FIXED: Better date formatting with multiple format attempts
                        created_date = None
                        ...
                        if test["created"]:
                            ...
                            for fmt in date_formats:
                                try:
                                    created_date = datetime.strptime(test["created"], fmt)
                                    formatted_date = created_date.strftime("%B %d, %Y at %I:%M %p")
                                    break
                                except:
                                    continue
                            
                            # If none worked, use the raw string
                            if not created_date:
                                formatted_date = test["created"]
