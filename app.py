...
def get_user_progress(username):
    ...
    for r in rows:
        if r["username"] == username:
            return {
                "used": set(json.loads(r["used"])),
                "correct": set(json.loads(r["correct"])),
                "incorrect": set(json.loads(r["incorrect"])),
                "marked": set(json.loads(r["marked"]))
            }
...
def save_current_answer(test, q):
    # Helper to save the currently selected radio answer (used by Next, Previous, Save & Exit)
...
if st.session_state.page == "test":
    ...
    total_time = len(test["questions"]) * 90
    elapsed = int(time.time() - test["start"])
    remaining = max(0, total_time - elapsed)

    st.sidebar.metric("Time Used", f"{elapsed//60}:{elapsed%60:02d}")
    st.sidebar.metric("Time Left", f"{remaining//60}:{remaining%60:02d}")

    st.title(f"Question {test['index']+1}/{len(test['questions'])}")
    st.markdown(q["question"])

    # Use session_state key unique per question to preserve selection across reruns
    radio_key = f"q_{q['id']}_answer"
    choice = st.radio(
        "Select answer",
        list(q["options_map"].values()),
        index=None if q["id"] not in test["answers"] else
        list(q["options_map"].values()).index(
            q["options_map"][test["answers"][q["id"]]]
        ),
        key=radio_key
    )

    # Store current selection for save on Previous / Exit
    ...
    if col3.button("Save & Exit"):
        save_current_answer(test, q)
        st.session_state.page = "home"
        st.rerun()

    # ────────────────────────────────────────────────
    # Immediate feedback in Reading mode
    # ────────────────────────────────────────────────
    if test["mode"] == "Reading" and q["id"] in test["answers"]:
        user_letter = test["answers"][q["id"]]
        user_answer = q["options_map"][user_letter]
        correct_letter = q["answer"]
        correct_answer = q["options_map"][correct_letter]

        if user_letter == correct_letter:
            st.success(f"Correct! The answer is {correct_letter}")
        else:
            st.error(f"Incorrect. You chose {user_letter}, correct is {correct_letter}")
...
if st.session_state.page == "previous":
    ...
    if df.empty:
        ...
        fig = px.pie(
            ...
            values=[df["score"].sum(), df["total_questions"].sum() - df["score"].sum()],
