        ...
        if not user_tests:
                ...
                for i, test in enumerate(incomplete_tests):
                    with st.expander(f"⚠️ Incomplete Test {i+1}: {test.get('mode', 'Unknown')} Mode", expanded=(i==0)):
                        try:
                            ...
                            created_date = test.get("created", "Unknown date")
            ...
            if completed_tests:
                ...
                for i, test in enumerate(completed_tests):
                    with st.expander(f"Test {i+1}: {test.get('mode', 'Unknown')} Mode - Score: {test['score']}/{test['total_questions']}"):
                        try:
                            created_date = datetime.fromisoformat(test["created"]).strftime("%Y-%m-%d %H:%M")
                        except:
                            created_date = test.get("created", "Unknown date")
                        ...
                        st.write(f"**Score:** {test['score']}/{test['total_questions']}")
                        st.write(f"**System:** {test.get('system', 'All')}")
