...
def get_user_tests(username):
    try:
        ...
        # Sort by creation date (most recent first) - NEW: Proper date sorting
...
def save_test_session(username, test, completed=True):
    try:
        ...
        # Prepare test data for storage - simplified version
        test_data = {
            "answers": test["answers"],
            "questions": [
                {
                    "id": q["id"],
                    "question": q["question"][:500] if len(q["question"]) > 500 else q["question"],  # Limit length
                    "answer": q["answer"],
        ...
        try:
            ...
            if cell_list:
                # Update existing test (found by test_id)
                cell = cell_list[0]
                row = cell.row
...
def calculate_total_test_time(num_questions):
    ...
    total_seconds = num_questions * 90  # 90 seconds per question
...
if st.session_state.page == "home":
    ...
    if last_incomplete_test:
        ...
        if st.button("➡️ Continue Last Test", key="continue_last_home"):
            # Try to restore questions from original QUESTIONS list
            restored_questions = []
            for q_data in last_incomplete_test["questions"]:
                ...
                if not found:
                    # Create minimal question from stored data
...
if st.session_state.page == "previous_menu":
    ...
    with tab1:
        ...
        if not user_tests:
            ...
            if incomplete_tests:
                ...
                for i, test in enumerate(incomplete_tests):
                    with st.expander(f"⚠️ Incomplete Test {i+1}: {test.get('mode', 'Unknown')} Mode", expanded=(i==0)):
                        try:
                            # NEW: Better date formatting
                            created_date = datetime.fromisoformat(test["created"])
                            formatted_date = created_date.strftime("%B %d, %Y at %I:%M %p")
                        except:
                            formatted_date = test.get("created", "Unknown date")
            ...
            # Show completed tests - NEW: Sorted by date, most recent first
            if completed_tests:
                st.write(f"**Completed Tests ({len(completed_tests)})**")
                for i, test in enumerate(completed_tests):
                    # NEW: Calculate time per question if available
                    total_time = ""
                    try:
                    ...
                    with st.expander(f"Test {i+1}: {test.get('mode', 'Unknown')} Mode - Score: {test['score']}/{test['total_questions']}{total_time}"):
                        ...
                        st.write(f"**Score:** {test['score']}/{test['total_questions']} ({test['score']/test['total_questions']*100:.1f}%)")
                        st.write(f"**System:** {test.get('system', 'All')}")
...
if st.session_state.page == "create":
    ...
    # NEW: Display estimated test time for Test mode
    if mode == "Test":
...
if st.session_state.page == "test":
    ...
    if test["mode"] == "Test" and not test.get("is_review"):
        ...
        with col_head3:
            ...
            total_time = calculate_total_test_time(len(test["questions"]))
            warning_threshold = total_time * 0.1  # Last 10% of time
    ...
    # Auto-refresh for timer in Test mode
    if test["mode"] == "Test" and not test.get("is_review"):
        time.sleep(1)  # Wait 1 second
        st.rerun()  # This will refresh the timer display
...
if st.session_state.page == "review":
    ...
    with col3:
        # Display time taken if in Test mode
