...
def get_user_tests(username):
    try:
        ...
        for r in rows:
            if r.get("username") == username:
                # Try to parse test_data - handle both string and dict formats
...
def save_test_session(username, test, completed=True):
    try:
        ...
        # Prepare test data for storage - only store essential info
        test_data = {
        ...
        try:
            existing_cell = tests_ws.find(test["id"], in_column=2)
            # Update existing test
            row = existing_cell.row
            tests_ws.update(f"C{row}:J{row}", [[
                ...
                str(completed).lower(),
                "updated"  # Add a flag to indicate this was updated
            ]])
            ...
            tests_ws.append_row([
                ...
                json.dumps(test_data),
                str(completed).lower(),
                "new"  # Add a flag to indicate this was new
            ])
...
if st.session_state.page == "home":
    ...
    for test in user_tests:
        if not test.get("completed", True):  # Changed from False to check incomplete
    ...
    if last_incomplete_test:
        ...
        st.write(f"**System:** {last_incomplete_test.get('system', 'All')}")
        if st.button("➡️ Continue Last Test", key="continue_last"):
            # Restore test session with proper question reconstruction
            restored_questions = []
            for q_data in last_incomplete_test["questions"]:
                # Try to find the original question in QUESTIONS
                original_q = None
                for q in QUESTIONS:
                    if q["id"] == q_data.get("id"):
                        original_q = q.copy()
                        break
                ...
                if original_q is None:
                    ...
                    original_q = {
                        ...
                        "options_map": q_data.get("options_map", {}),
                        "options": list(q_data.get("options_map", {}).values()),
...
if st.session_state.page == "previous_menu":
    ...
    with tab1:
        st.subheader("Your Previous Tests")
        user_tests = get_user_tests(st.session_state.user)
        ...
        if not user_tests:
            ...
            if completed_tests:
                ...
                # Display as expandable sections instead of dataframe
                for test_info in test_data:
                    with st.expander(f"Test {test_info['Test #']}: {test_info['Date']} - {test_info['Mode']} Mode - {test_info['Score']}"):
                        st.write(f"**Date:** {test_info['Date']}")
                        st.write(f"**Mode:** {test_info['Mode']}")
                        st.write(f"**Questions:** {test_info['Questions']}")
                        st.write(f"**Score:** {test_info['Score']}")
                        st.write(f"**System:** {test_info['System']}")
                        st.write(f"**Status:** {test_info['Status']}")
                        
                        # Find the actual test data
                        test_idx = test_info['Test #'] - 1
                        if test_idx < len(completed_tests):
                            test_to_review = completed_tests[test_idx]
                            if st.button(f"Review This Test", key=f"review_{test_info['Test #']}"):
                                # Restore the test for review
                                restored_questions = []
                                for q_data in test_to_review["questions"]:
            ...
            if incomplete_tests:
                ...
                for i, test in enumerate(incomplete_tests):
                    ...
                    with st.expander(f"⚠️ Incomplete Test: {created_date} - {test['mode']}"):
                        st.write(f"**Progress:** Question {test['index'] + 1}/{test['total_questions']}")
                        st.write(f"**System:** {test.get('system', 'All')}")
                        
                        if st.button(f"Continue This Test", key=f"continue_incomplete_{i}"):
                            # Restore incomplete test
                            restored_questions = []
    ...
    with tab2:
        ...
        st.subheader("Question Usage")
        usage_percent = (used_questions / total_questions * 100) if total_questions > 0 else 0
        st.progress(usage_percent / 100)
        st.write(f"**{used_questions}/{total_questions}** questions used ({usage_percent:.1f}%)")
        ...
        if completed_tests:
            ...
            for test in completed_tests[-10:]:  # Last 10 tests
                try:
                    ...
                    score_pct = (test["score"] / test["total_questions"]) * 100 if test["total_questions"] > 0 else 0
    ...
    with tab3:
        ...
        if incomplete_tests:
            # Show all incomplete tests, not just the first one
            st.write(f"Found {len(incomplete_tests)} incomplete test(s):")
            ...
            for i, test in enumerate(incomplete_tests):
                ...
                with st.expander(f"Incomplete Test {i+1}: {created_date}"):
                    st.write(f"**Test Mode:** {test['mode']}")
                    st.write(f"**Progress:** Question {test['index'] + 1}/{test['total_questions']}")
                    ...
                    with col2:
                        if st.button(f"Restart", key=f"restart_{i}"):
                            # Create new test with same questions
                            restored_questions = []
                            for q_data in test["questions"]:
                                original_q = None
                                for q in QUESTIONS:
                            ...
                            if restored_questions:
                                # Create new test ID for restart
                                st.session_state.test = {
                                    "id": str(uuid.uuid4()),
                                    "questions": restored_questions,
                                    "answers": {},
                                ...
                                # Mark old test as completed
                                save_test_session(st.session_state.user, {
                                    "id": test["test_id"],
                                    "questions": restored_questions,
                                    "answers": test["answers"],
                                    "marked": test["marked"],
            ...
            st.info("No incomplete tests found. All your tests are completed!")
    ...
    with col2:
        if st.button("Start Test", use_container_width=True):
            if len(pool) < num_q:
                ...
                # Save initial test session as incomplete
